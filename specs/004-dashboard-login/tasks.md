...
- [X] T001 Create `internal/dashboard/session.go` — `//go:build windows` tag, `package dashboard`, empty imports block
- [X] T002 [P] Create `internal/dashboard/login_handler.go` — `//go:build windows` tag, `package dashboard`, empty imports block
- [X] T003 [P] Create `frontend/src/lib/auth.svelte.js` — empty Svelte 5 module with exported `authState` placeholder
...
- [X] T004 Implement `Session` struct, `SessionStore` (Create/Get/Delete, 8 h idle + 24 h absolute expiry, 15-min background reaper) and `requireSession(store *SessionStore) func(http.Handler) http.Handler` middleware in `internal/dashboard/session.go` — see data-model.md for fields and cookie contract
- [X] T005 [P] Add `sessionStore *SessionStore` field to server struct and initialize it (via `NewSessionStore(ctx)`) in `internal/dashboard/server.go`
- [X] T006 [P] Scaffold `authState` reactive object, and stub exports `probeNegotiate`, `loginWithCredentials`, `logout` (empty async functions) in `frontend/src/lib/auth.svelte.js`
- [X] T007 Update `apiFetch` in `frontend/src/lib/api.js` — add 401 intercept that sets `authState.username = null` and `authState.error = 'session_expired'` before re-throwing
...
- [X] T018 [P] [US3] Implement `handleLogout(store *SessionStore) http.HandlerFunc` in `internal/dashboard/login_handler.go` — reads `drainctl_session` cookie; if present and found in store, calls `store.Delete(token)`; always responds with `Set-Cookie: drainctl_session=; Max-Age=0; HttpOnly; SameSite=Strict; Path=/` and `200 {"ok":true}` (idempotent — missing or unknown cookie is not an error)
- [X] T019 [US3] Add `POST /api/v1/auth/logout` route in `internal/dashboard/server.go` — no auth wrapper; handler is `handleLogout(s.sessionStore)`
- [X] T020 [P] [US3] Implement `logout()` in `frontend/src/lib/auth.svelte.js` — POSTs to `/api/v1/auth/logout`; on response (any status) sets `authState.username = null`, clears `authState.error`, sets `authState.skipProbe = true` (new flag) so `App.svelte` shows the login form without triggering a new auto-login probe
- [X] T021 [US3] Add Sign Out button to `frontend/src/components/Nav.svelte` — visible only when `authState.username` is set; calls `logout()` on click; neobrutal style matching existing nav buttons (solid 2 px border, no fill, same height); position after the existing CONFIG button
...
- [X] T022 [P] Add mock auth endpoints to `frontend/dev/mock-api.js` — `POST /api/v1/auth/negotiate` returns `200 {"username":"DEV\\mockuser"}`; `POST /api/v1/auth/login` with non-empty username/password returns `200 {"username": body.username}` else `401 {"error":"invalid credentials"}`; `POST /api/v1/auth/logout` returns `200 {"ok":true}`
- [X] T023 [P] Review `internal/dashboard/auth_dev.go` — confirm `//go:build windows && devmode` tag doesn't conflict with session.go; `requireSession` in dev mode should be a pass-through no-op (add to `auth_dev.go` if the production implementation's build tag excludes devmode)
- [X] T024 Run `just lint` and `just all` to confirm zero warnings and successful build across all targets
