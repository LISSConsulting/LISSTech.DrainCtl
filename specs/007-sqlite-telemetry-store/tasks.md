...
- [ ] T059a Verify removals complete in Go source: `rg --type go MemAuditStore` returns zero hits repo-wide; `rg --type go "history\s+map\["` scoped to `internal/dashboard/` returns zero hits. Historical references in `specs/`, `CHRONICLE.md`, and `docs/reviews/` are expected and out of scope — the gate is production Go code only. Additionally update the stale architecture line in `CLAUDE.md` (currently `MemAuditStore = in-memory + JSONL flush. AuditStore = file-only (CLI fallback).`) to describe the telemetry store instead, so future loops don't re-introduce the removed types from a stale mental model. (Use `rg` / `Select-String` — do NOT use bash `grep`; project builds on Windows and contributors may not have a POSIX shell.) Covers SC-009.
