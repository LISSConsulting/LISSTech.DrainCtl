...
## Decision 10: Version transitions logged to slog.Info only; durable persistence DEFERRED

- **Decision (revised mid-implementation)**: v1 records each successful version transition via a single `slog.Info` line with fields `update=installing old=V0 new=Vlatest`. Durable persistence to the SQLite audit store is **deferred to a follow-up spec**.
- **Rationale**: The original draft of this decision (and of data-model.md §2) claimed the existing `telemetry.AuditStore.Append` takes a free-form `event string` — that claim was wrong. `internal/telemetry/audit.go` defines a drain-mode-specific schema (int `prev_state`/`new_state` columns, no `event` or `metadata` columns) keyed by `(ts, host, new_state)`. Wiring `auto_update_install` into that schema requires one of: (a) extending `AuditRecord` and the SQL with optional event+metadata fields, (b) adding a sibling `events` table, or (c) repurposing existing fields with sentinel int values. Each is a real design decision that does not belong inside this PR — they affect every audit caller and the audit table's invariants. The 7-day file-log rotation gives operators visibility for v1; a fleet operator who needs longer history can grep older rotated logs from the file-log archive directory.
- **Alternatives considered**:
  - **Ship the audit row via Option (a) — extend AuditRecord**: rejected mid-implementation — the audit-table redesign is a separate decision affecting all audit callers, not just the updater.
  - **Repurpose existing fields (Option c)**: rejected — encoding `event=auto_update_install` into `Reason` and using sentinel ints for `prev_state`/`new_state` breaks the audit table's "drain-mode transition" invariant. Other audit consumers (CLI `drainctl history`, dashboard views) assume the int states are real.
  - **New `update_history` table in the telemetry DB**: rejected — premature normalization for one row per ~weeks-to-months interval, but this is the most likely shape of the future durable record.
  - **Add the audit row in a small follow-up commit on this same PR**: rejected — keeps the audit-table redesign in scope of this PR's review, defeating the "ship self-poll cleanly" goal.

The schema mismatch was caught by the agent implementing Commit 9, before any updater code that depended on the audit-row contract was written. Spec.md FR-011 and data-model.md §2 were updated to reflect the deferred status; tasks.md Commit 9 was updated to drop the AuditAppender interface entirely.
