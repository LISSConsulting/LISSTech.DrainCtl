...
- **FR-010**: The updater MUST keep the current service running after a successful installer spawn. Windows Installer owns the stop/install/start transition through the MSI's `ServiceControl` and restart actions; a spawn is not proof that installation completed, so the updater MUST NOT cancel the service-level context, retain the release ETag, or advance `highest_seen` before the new binary starts.
