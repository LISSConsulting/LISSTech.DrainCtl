...
- **FR-016**: `internal/svc/handler.go` MUST split into topically-coherent files (`handler.go` core Run loop, `handler_pipe.go`, `handler_dashboard.go`, `handler_perf.go`, `handler_evtspike.go`, `handler_lifecycle.go`) with no public signature changes.
