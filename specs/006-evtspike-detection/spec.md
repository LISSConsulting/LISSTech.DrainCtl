...
- Q: What dashboard visibility must MVP ship with? → A: Additive-minimal on the existing dashboard — per-server detector status pill (healthy / training / disabled / error) plus a recent-spikes list for the selected server (the list requires the unified telemetry store in spec 007 to be present; when 007 is absent the list surface is omitted and core detection/notification continue unchanged). No new dashboard construction; no per-channel drilldown, heatmap, or baseline-editing UI in MVP.
...
5. **Given** the administrator opens the detail view for a server **and** the unified telemetry store (spec 007) is present, **When** confirmed spikes have occurred for that server, **Then** a recent-spikes list shows the last several entries (channel, time, observed vs. expected count) using the dashboard's existing list styling. **Given** the same detail view on an installation without 007, **Then** the list is omitted and the rest of the detail view renders normally.
...
- **FR-012a**: Confirmed spike events MUST be emitted by the scoring engine through an internal in-process sink interface. Zero or more **Spike Event Sinks** subscribe to this interface; each sink is responsible for its own downstream effect. At MVP, two sinks exist: the notification-pipeline sink (always registered) and the telemetry-store persistence sink (registered only when spec 007 is present). The scoring engine MUST remain indifferent to sink identity, count, and ordering.
- **FR-012b**: The scoring engine MUST NOT write directly to the unified telemetry store (spec 007) and MUST NOT import, reference, or link against the telemetry-store persistence layer. All durable persistence of spike events is the responsibility of the telemetry-store persistence sink, which is an adapter *outside* the engine boundary. This preserves the reusable scoring engine's portability (see Architectural Note) and guarantees that engine unit tests are runnable with an in-memory fake sink — no SQLite fixture, no filesystem, no 007 dependency required.
...
- **FR-027**: When the unified telemetry store (spec 007) is present, each server's detail view on the existing dashboard MUST include a recent-spikes list showing the most recent confirmed spikes for that server: channel name, confirmation time, observed count, expected count. The list MUST reuse the dashboard's existing list styling and is backed by 007's Spike Event Record query surface (007 FR-016a), so it survives service restart and is not limited to in-memory retention. When 007 is absent (pre-upgrade installations), the absence is logged once at subsystem start and the detail view omits the list rather than showing empty or erroring — core detection and notification delivery continue to operate normally.
...
- **Spike Event**: The detector's output. Carries server, channel, observed count, expected count, and window timestamps. Severity is assigned by notification-target wiring (FR-011a), not by the detector; the Spike Event itself does not carry a severity field. Delivered in-process to every registered Spike Event Sink (see below). Sinks include the notification pipeline (always) and, when the unified telemetry store (spec 007) is present, the persistence sink that inserts a durable Spike Event Record so the FR-027 recent-spikes list survives restart. Persisted Spike Event Records are immutable after insertion; the only deletion path is 007's bulk retention job acting on age.
...
- **Spike Event Sink**: An internal, in-process subscriber to the scoring engine's confirmed-spike output (FR-012a). Responsible for one downstream effect — notification dispatch, durable persistence to the unified telemetry store, or other future consumers. Multiple sinks MAY be registered concurrently; the engine is indifferent to sink identity. MVP ships two sinks: the notification-pipeline sink (always registered) and the telemetry-store persistence sink (registered only when spec 007 is present). Sinks MUST NOT be reimplemented inside the engine; they are adapters *outside* the engine boundary so that future scoring-engine consumers (e.g., the perf-counter adapter described in "Future Considerations") can bring their own sinks without touching engine code.
...
- **Telemetry store dependency (spec 007)**: This feature depends on the unified telemetry store (spec 007) being present for two purposes only: (a) durable persistence of confirmed spike events as Spike Event Records, and (b) the dashboard recent-spikes list surface (FR-027). Core detection, scoring, confirmation, cooldown, and notification delivery do NOT depend on 007 — they continue to operate on installations that have not yet been upgraded to include 007. The **baseline state** remains on disk as a single atomic-replace JSON file (FR-018..FR-021) and does NOT live in 007; this is deliberate to keep baseline persistence independent of the telemetry store's availability and to preserve the scoring engine's portability.
...
- **Engine-owned cross-cutting concerns.** Baseline storage, time-of-day slotting, per-slot maturity tracking, 2-of-3 confirmation logic, and cooldown management remain engine responsibilities. Adapters MUST NOT reimplement these. Conversely, **durable persistence of spike events is NOT an engine responsibility** — it is performed by a Spike Event Sink (FR-012a, FR-012b) registered outside the engine. The engine MUST NOT open, import, or reference the unified telemetry store (spec 007) or any SQL driver.
- **Source-agnostic spike output.** The spike event emitted by the engine carries source ID, source type, metric name, observed value, expected value, and window timestamps. The event is delivered to every registered Spike Event Sink; the notification-pipeline sink formats the human-readable message based on source type, the telemetry-store persistence sink writes a durable record, and future adapters (perf counters, custom metrics) inherit the same sink contract without modifying the engine. The engine itself is indifferent to what the source represents and to which sinks are registered.
