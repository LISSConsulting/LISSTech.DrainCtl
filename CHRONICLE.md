...
**Decision.** Let Windows SCM remain the sole recovery authority: restart the first and second unexpected failures after five seconds, take no third recovery action, and reset the count after one day. Capture bounded crash evidence through WER LocalDumps—not an application crash-path uploader—and make it local-only under a SYSTEM/Administrators-only ProgramData boundary. Treat sparse/inactive RemoteFX separately from malformed telemetry, and make offline transitions durable state boundaries rather than ephemeral dashboard timers.

**Implementation.**

- The installer provisions the exact SCM failure-action policy and a permanent protected `dumps` directory. WER LocalDumps applies only to `drainctld.exe`, writes mini dumps, and retains at most three. The companion permanent protected `diags` directory receives the installed hourly diagnostics when its disabled-by-default task is enabled: service state/failure actions, WER configuration, relevant crash events, and metadata-only dump inventory. Dump bytes are not copied, uploaded, stored in SQLite, or emitted through logs, notifications, dashboard APIs, or SSE.
- `DRAINCTL_INCLUDE_CRASH_DUMPS=1` is an explicit local-investigation gate. It hashes and gzip-copies only the newest dump inside protected `dumps`; its archive and sidecar never enter `diags` and are never uploaded. WER continues to own `.dmp` retention, so product cleanup never races it.
- Dashboard freshness is SQLite schema v3, keyed by canonical host and accepted report epoch. Offline is exactly three effective heartbeat intervals; a poll-interval reload wakes the timer immediately. The dashboard emits additive `host_offline` and `host_recovered` events once per outage/recovery while retaining `server_update`, and does not rewrite `last_seen` merely to evaluate freshness.
- Shared RemoteFX validation applies before local or remote data can influence server state, aggregates, or persistence. FPS is `(0,240]`, quality `(0,100]`, encode/RTT `[0,60000]`, loss `[0,100]`, and skip rates `[0,1000000]`; NaN, infinity, negative, and outlier fields are dropped independently. Zero FPS/quality is an inactive-stream gap, while valid lower-is-better zeroes—including P50—persist.

**Migration and compatibility.** The freshness schema upgrade is restart-safe; existing hosts without new report fields remain valid, and older dashboards may ignore additive fields. Existing `server_update` consumers continue unchanged. Upgrades preserve configuration, SQLite telemetry, and protected dump/diagnostic artifacts. Full uninstall removes the WER policy and diagnostics task but intentionally retains `dumps` and `diags`; rollback likewise does not authorize artifact deletion. Operators should inspect a missing dump as missing evidence, not proof of a particular crash location.
...
- **App CalVer `YY.MM.BUILD` is git-derived, nothing to bump by hand.** `scripts/version.ps1` computes it from the commit month + monthly commit count; `just all`/`just release` inject it into Go, PE resources, package naming, and the PowerShell module. MSI `ProductVersion` deliberately uses `(100 + YY).MM.BUILD`: old day-of-year packages such as `26.117.14` numerically outrank `26.9.*` under Windows Installer rules, so the `100` major-version epoch makes the monthly scheme newer and makes old packages' own downgrade checks reject replay.
