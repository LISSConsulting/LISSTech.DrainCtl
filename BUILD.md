...
3. **Fallback backlog — `BUGS.md`.** If every task in the active feature is `- [x]` (or `.specify/feature.json` is empty / absent), open `BUGS.md`. Pick the first numbered `### N.` entry whose block does **NOT** carry a `**Status:**` line of `Fixed.`, `Won't fix — …`, or `Duplicate of #M`. BUGS.md entries are self-contained (Surfaced / Symptom / Root cause / Fix / Scope); treat the whole entry as your task description. Mark this iteration's source as `BUGS.md#N` so the commit message + status-update (step 18a) target the right place.
