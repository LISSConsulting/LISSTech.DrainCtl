...
/**
 * Seed per-server metric ring buffers from retained SQLite history on startup.
 * Called once (before live polls accumulate); overwrites any stale browser-local
 * data so the backend is authoritative at cold start.
 *
 * A single Map replacement avoids creating N intermediate Maps for large fleets.
 *
 * @param {Map<string, import('./api.js').MetricsSeedSample[]>} seedData
 */
