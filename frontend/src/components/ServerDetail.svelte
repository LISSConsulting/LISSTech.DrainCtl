...
<div class="d-inner">
    ...
    {#if !isOff}
      ...
      <div class="d-ring-row">
        <div class="d-ring-cell">
          <RingGauge value={sessionsPct} max={100} label="Sessions" unit=" "
            warnThreshold={sessionWarnThresh} critThreshold={sessionCritThresh}
            centerLabel={String(server.sessions)} size={72} />
        </div>
        <div class="d-ring-cell">
          <RingGauge value={perf.cpu_pct} label="CPU" unit="%" warnThreshold={cpuThresh.warn} critThreshold={cpuThresh.crit} size={72} />
        </div>
        <div class="d-ring-cell">
          <RingGauge value={memPct} label="Memory" unit="%" warnThreshold={memThresh.warn} critThreshold={memThresh.crit} size={72} />
        </div>
      </div>
      ...
      <div class="d-legend">
        <div class="d-leg-col">
          ...
          <div class="d-leg-row"><span class="d-leg-k">Active</span><span class="d-leg-v">{server.sessions_active ?? '—'}</span></div>
          <div class="d-leg-row"><span class="d-leg-k">Disconn.</span><span class="d-leg-v">{server.sessions_disconnected ?? '—'}</span></div>
          <div class="d-leg-row"><span class="d-leg-k">Total/Max</span><span class="d-leg-v">
        ...
        <div class="d-leg-col">
          ...
          <div class="d-leg-row"><span class="d-leg-k">Host</span><span class="d-leg-v">{perf.cpu_pct != null ? perf.cpu_pct.toFixed(0) + '%' : '—'}</span></div>
          <div class="d-leg-row"><span class="d-leg-k has-tip">P95<span class="tip">95th percentile across all sessions — 95% of sessions use less than this value.</span></span><span class="d-leg-v">{perf.session_cpu_p95_pct != null && perf.session_cpu_p95_pct > 0 ? perf.session_cpu_p95_pct.toFixed(0) + '%' : '—'}</span></div>
        </div>
        <div class="d-leg-col">
  ...
  <div class="d-tile d-tile-io">
    ...
    {#if !isOff}
      ...
      <div class="d-ring-row">
        <div class="d-ring-cell">
          <RingGauge value={perf.disk_queue} max={DEFAULTS.diskQueue.crit * 2}
            label="Disk Q" unit=" " warnThreshold={DEFAULTS.diskQueue.warn} critThreshold={DEFAULTS.diskQueue.crit} size={72} />
        </div>
        <div class="d-ring-cell">
          <RingGauge value={perf.input_delay_p95_ms} max={delayThresh.crit * 2}
            label="Inp Dly" unit="ms" warnThreshold={delayThresh.warn} critThreshold={delayThresh.crit} size={72} />
        </div>
        <div class="d-ring-cell">
          <RingGauge value={perf.tcp_retrans_sec} max={DEFAULTS.tcpRetransmits.crit * 2}
            label="TCP Rx" unit="/s" warnThreshold={DEFAULTS.tcpRetransmits.warn} critThreshold={DEFAULTS.tcpRetransmits.crit} size={72} />
        </div>
      </div>
      ...
      <div class="d-legend">
        <div class="d-leg-col">
          ...
          <div class="d-leg-row"><span class="d-leg-k has-tip">P50<span class="tip">Median — half of all measurements are below this. Represents the typical user experience.</span></span><span class="d-leg-v">{perf.input_delay_p50_ms != null ? perf.input_delay_p50_ms.toFixed(0) + ' ms' : '— ms'}</span></div>
          <div class="d-leg-row"><span class="d-leg-k has-tip">P95<span class="tip">95th percentile — 95% of measurements below this. Shows worst experience most users face.</span></span><span class="d-leg-v">{perf.input_delay_p95_ms != null ? perf.input_delay_p95_ms.toFixed(0) + ' ms' : '— ms'}</span></div>
          <div class="d-leg-row"><span class="d-leg-k">Max</span><span class="d-leg-v">{perf.input_delay_max_ms != null ? perf.input_delay_max_ms.toFixed(0) + ' ms' : '— ms'}</span></div>
        </div>
        <div class="d-leg-col">
          <div class="d-leg-head">Disk &amp; Net</div>
          <div class="d-leg-row"><span class="d-leg-k">Pages/sec</span><span class="d-leg-v">{perf.pages_sec != null ? perf.pages_sec.toFixed(1) : '—'}</span></div>
          <div class="d-leg-row"><span class="d-leg-k">Disk Queue</span><span class="d-leg-v">{perf.disk_queue != null ? perf.disk_queue.toFixed(2) : '—'}</span></div>
          <div class="d-leg-row"><span class="d-leg-k">TCP Retrans</span><span class="d-leg-v">{perf.tcp_retrans_sec != null ? perf.tcp_retrans_sec.toFixed(1) + '/s' : '—'}</span></div>
        </div>
      </div>
    {:else}
  ...
  <div class="d-tile d-tile-details">
    ...
    {#if !isOff}
      <div class="d-kv-row"><span class="d-kv-k">Drain Mode</span><span class="d-kv-v">{modeLabel(server.drain_mode)}</span></div>
      <div class="d-kv-row"><span class="d-kv-k">State Since</span><span class="d-kv-v">{stateSinceStr}</span></div>
      <div class="d-kv-row"><span class="d-kv-k">Duration</span><span class="d-kv-v">{dur(server.state_duration_seconds)}</span></div>
      {#if gracePeriodSec != null}
        <div class="d-kv-row"><span class="d-kv-k">Grace Period</span><span class="d-kv-v">{Math.round(gracePeriodSec / 60)}m</span></div>
      {/if}
      {#if graceLeft != null && graceLeft > 0}
        <div class="d-kv-row"><span class="d-kv-k">Grace Left</span><span class="d-kv-v" style="color:var(--color-amber)">{dur(graceLeft)}</span></div>
      {/if}
      {#if graceExceeded != null && graceExceeded > 0}
        <div class="d-kv-row"><span class="d-kv-k">Grace Exceeded</span><span class="d-kv-v" style="color:var(--color-red)">{dur(graceExceeded)} ago</span></div>
      {/if}
      <div class="d-kv-row"><span class="d-kv-k">Changed By</span><span class="d-kv-v">{server.changed_by || '—'}</span></div>
    {/if}

    <div class="d-kv-row"><span class="d-kv-k">Last Seen</span><span class="d-kv-v" style="color:{lsColor}">{rel(server.last_seen)}</span></div>
    <div class="d-kv-row"><span class="d-kv-k">Version</span><span class="d-kv-v">{server.version || '—'}</span></div>
    <div class="d-kv-row"><span class="d-kv-k">Registered</span><span class="d-kv-v">{rel(server.registered_at)}</span></div>

    <!-- Action buttons -->
    <div class="d-actions">
      {#if onhistoryclick}
        <button class="d-btn d-btn-hist" onclick={() => onhistoryclick(server.host)}>History</button>
      {/if}
      {#if onremove}
        <button class="d-btn d-btn-rm" onclick={() => onremove(server.host)}>Remove</button>
      {/if}
    </div>
  </div>
</div>
...
<style>
  ...
  .d-tile-util    { flex: 1.5; min-width: 280px; }
  .d-tile-io      { flex: 1.3; min-width: 260px; }
  .d-tile-details { flex: 1; min-width: 180px; }
  ...
  .d-leg-k { color: var(--color-muted); }
  .d-leg-v { font-weight: 600; }
  ...
  .d-kv-v { font-weight: 600; text-align: right; }
