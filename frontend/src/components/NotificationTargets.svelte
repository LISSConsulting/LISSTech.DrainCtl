<script>
    ...
    function openEdit(idx) {
        ...
        editTarget =
            idx >= 0
                ? JSON.parse(JSON.stringify(targets[idx]))
                : {
                      type: 'webhook',
                      url: '',
                      to: [],
...
<div class="settings-group">
    ...
    <div class="target-tbl-wrap">
        <table class="target-tbl">
            ...
            <tbody>
                {#if !targets?.length}
                    <tr class="empty-row"
                        ><td colspan="6" class="target-tbl-empty">No notification targets configured.</td></tr
                    >
                ...
                {:else}
                    {#each pagedItems as { t, idx }}
                        ...
                        <tr>
                            ...
                            <td
                                ><span
                                    class="pill-type {t.type === 'ntfy'
                                        ? 'pill-type-ntfy'
                                        : t.type === 'email'
                                          ? 'pill-type-email'
                                          : ''}"
                                    >{t.type === 'ntfy' ? 'Ntfy' : t.type === 'email' ? 'Email' : 'Webhook'}</span
                                ></td
                            >
                            <td class="td-dest" title={t.type === 'email' ? (t.to || []).join(', ') : t.url}
                                >{t.type === 'email' ? (t.to || []).join(', ') : t.url}</td
                            >
                            <td>
                                {#if trigs.length <= 2}
                                    <span class="tgt-triggers"
                                        >{#each trigs as tr}<span class="tgt-pill">{TRIGGER_LABELS[tr] || tr}</span
                                            >{/each}</span
                                    >
                                {:else}
                                    <span class="tgt-triggers"
                                        ><span class="tgt-pill">{TRIGGER_LABELS[trigs[0]] || trigs[0]}</span><span
                                            class="tgt-pill tgt-pill-more"
                                            title={trigs.map((tr) => TRIGGER_LABELS[tr] || tr).join(', ')}
                                            >+{trigs.length - 1}</span
                                        ></span
                                    >
                                {/if}
                            ...
                            <td>
                                <div class="btn-row">
                                    <button class="btn-tbl" onclick={() => openEdit(idx)}
                                        ><Pencil size={12} /> Edit</button
                                    >
                                    <button class="btn-tbl btn-tbl-danger" onclick={() => (deleteIdx = idx)}
    ...
    <div class="tgt-footer">
        ...
        <div class="tgt-pager" style={totalPages > 1 ? '' : 'visibility:hidden'}>
            ...
            <button class="btn-page" disabled={page >= totalPages - 1} onclick={() => page++}
...
<style>
    ...
    .btn-page:hover {
        transform: translate(-1px, -1px);
        box-shadow: calc(var(--spacing-so) + 1px) calc(var(--spacing-so) + 1px) 0 var(--color-shadow);
    }
    .btn-page:active {
        transform: translate(1px, 1px);
        box-shadow: 1px 1px 0 var(--color-shadow);
