<script>
  ...
  let activeFireLevel = $derived.by(() => {
    ...
    for (const pr of FIRE_PRESETS) {
      if (config.grace_period === pr.grace_period &&
          config.session_warning_threshold === pr.session_warning &&
          p?.cpu_warn_pct === pr.cpu_warn && p?.cpu_crit_pct === pr.cpu_crit &&
          p?.mem_warn_pct === pr.mem_warn && p?.mem_crit_pct === pr.mem_crit &&
          p?.input_delay_warn_ms === pr.delay_warn && p?.input_delay_crit_ms === pr.delay_crit) {
        return pr.level;
      }
    }
    return -1;
      ...
      <div class="settings-actions-wrap">
        <div class="settings-actions">
          <button class="btn-brutal btn-test" onclick={sendTest} disabled={testing}>
            <Play size={14} /> {testing ? 'Sending...' : 'Send Test'}
          </button>
          <div style="display:flex;gap:8px">
            <button class="btn-brutal btn-save" onclick={save} disabled={saving || !dirty}>
              <Save size={14} /> {saving ? 'Saving...' : 'Save'}
            </button>
            <button class="btn-brutal btn-secondary" onclick={requestClose}><X size={14} /> Close</button>
          </div>
        </div>
      </div>
    {/if}
...
<style>
  ...
  .settings-modal { max-height: 90vh; background: var(--color-card); border: 4px solid var(--color-border); border-radius: var(--radius-default); box-shadow: 10px 10px 0 var(--color-shadow); overflow-y: auto; padding: 32px 36px; transition: background 0.3s; animation: modal-card-in 0.2s ease-out; }
  ...
  .fire-icon-wrap { color: var(--color-accent); line-height: 1; margin-bottom: 2px; }
  .fire-card.active .fire-icon-wrap { color: var(--color-red); }
  ...
  .btn-save { display: inline-flex; align-items: center; gap: 5px; background: var(--color-accent); color: #fff; padding: 10px 22px; font-family: 'Work Sans', sans-serif; font-size: 0.85rem; font-weight: 700; }
  ...
  .btn-test { display: inline-flex; align-items: center; gap: 5px; background: var(--color-card); color: var(--color-fg); padding: 10px 22px; font-family: 'Work Sans', sans-serif; font-size: 0.85rem; font-weight: 700; }
  ...
  .btn-secondary { display: inline-flex; align-items: center; gap: 5px; background: var(--color-card); color: var(--color-fg); padding: 10px 22px; font-family: 'Work Sans', sans-serif; font-size: 0.85rem; font-weight: 700; }
</style>
